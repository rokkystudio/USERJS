...
(() => {
     ...
     * При ручном выборе модели обычный primary_assistant ход остаётся Chat:
     * задаётся primary_assistant, явно передаётся conversation_origin=null и
     * исключается Work execution target. Метаданные нового user-сообщения также
     * не передают conversation_execution_target. Выбор Auto не меняет режим
     * разговора и model slug. Gizmo-режимы не преобразуются.
     *
     * @param {string} body
    ...
    function formatRequestStatus(requestInfo) {
        ...
        const modeText = state.selectedModelSlug !== config.autoModelSlug
    ...
    /**
     * Добавляет панель выбора модели, thinking effort, скорости и диагностики.
     *
     * Все параметры применяются сразу при изменении. При ручном выборе model slug
     * исходящие primary_assistant ходы автоматически сохраняются в режиме Chat.
     */
    function createPanel() {
        ...
        const hint = createElement('div', { class: 'gpt-model-picker-hint' }, 'Выбор модели применяется сразу. Для любого ручного slug обычный primary_assistant запрос остаётся Chat: conversation_origin=null, Work execution target не передаётся. Auto не меняет model slug и режим разговора.');
        ...
        panel.append(header, modelLabel, inputLabel, thinkingLabel, fastLabel, diagnostics, hint);
