...
(() => {
    ...
    const state = {
        ...
        restoringChat: false,
        pendingDirectWorkChatVerification: false,
        stopped: false
    };
     ...
     * явно передаёт conversation_origin=null и исключает Work execution target
     * из исходящего хода. Метаданные нового user-сообщения также не передают
     * conversation_execution_target. Явный null позволяет backend отличить
     * запрос Chat от отсутствующего поля origin. Gizmo-режимы не преобразуются.
     *
     * @param {string} body
    ...
    async function observeConversationResponse(response, requestInfo) {
        ...
        if (!responseBody) {
            ...
            if (requestInfo.verifyDirectWorkChat) {
                await verifyDirectWorkChatAfterResponse();
            }
        ...
        try {
            ...
            if (requestInfo.verifyDirectWorkChat) {
                try {
                    ...
                    setStatus(state.restoreChatStatus, `Chat: ошибка проверки server origin: ${error.message}`, 'error');
                    log('direct Work Chat verification failed', error);
                }
            }
        }
    }
    ...
    async function fetchWithSelectedModel(input, init) {
        ...
        const verifyDirectWorkChat = state.pendingDirectWorkChatVerification;
        ...
        if (verifyDirectWorkChat && response.ok) {
            state.pendingDirectWorkChatVerification = false;
            requestInfo.verifyDirectWorkChat = true;
        }
     ...
     * явно передаёт conversation_origin=null и сохраняет выбранный model slug.
     * Server-side тип проверяется после хода, подготовленного кнопкой Work → Chat.
     *
     * @param {boolean} enabled
     * @param {boolean} persist
     */
    ...
    /**
     * Проверяет server-side тип разговора после хода, подготовленного кнопкой Work → Chat.
     *
     * Проверка выполняется после завершения потокового ответа. При origin=null
     * страница перезагружается, чтобы frontend перечитал разговор уже как Chat.
     * При сохранённом tpp статус явно сообщает, что backend оставил разговор Work.
     *
     * @returns {Promise<void>}
     */
    async function verifyDirectWorkChatAfterResponse() {
        const conversationId = getCurrentConversationId();

        if (!conversationId) {
            setStatus(state.restoreChatStatus, 'Chat: после хода conversation id не найден', 'warning');
            return;
        }

        const conversationSummary = await findConversationSummary(conversationId);

        if (!conversationSummary) {
            setStatus(state.restoreChatStatus, 'Chat: после хода разговор не найден в server-side списке', 'warning');
            return;
        }

        if (conversationSummary.conversation_origin === 'tpp') {
            setStatus(state.restoreChatStatus, 'Chat: после хода backend сохранил server origin tpp', 'warning');
            return;
        }

        setStatus(
            state.restoreChatStatus,
            `Chat: server origin ${conversationSummary.conversation_origin ?? 'null'}; разговор подтверждён как Chat; перезагрузка…`,
            'success'
        );
    ...
    /**
     * Готовит следующий ход прямого Work-разговора как запрос обычного Chat.
     *
     * Прямой Work не имеет обратного handoff. Метод включает «Оставаться в Chat»;
     * следующий primary_assistant запрос явно передаёт conversation_origin=null.
     * После завершения потока server-side conversation_origin проверяется отдельно.
     */
    function armDirectWorkConversationForChat() {
        state.pendingDirectWorkChatVerification = true;
        setForceChatEnabled(true, true);
        setStatus(
            state.restoreChatStatus,
            'Chat: прямой Work; следующий ход отправится с conversation_origin=null',
            'success'
        );
    }

    /**
     * Переводит Work-разговор в доступное штатное продолжение Chat.
     *
     ...
     * Для прямого Work с conversation_origin=tpp включается «Оставаться в Chat».
     * Следующий primary_assistant ход явно передаёт conversation_origin=null, а
     * server-side origin проверяется после завершения потокового ответа.
     *
     * @returns {Promise<void>}
     */
        ...
        try {
            ...
            if (!result) {
                ...
                if (conversationOrigin === 'tpp') {
                    setStatus(
                        state.restoreChatStatus,
                        `Chat: прямой Work; server origin tpp${conversationSummary ? ' подтверждён списком разговоров' : ''}; подготовка следующего Chat-хода…`,
                        'neutral'
                    );
                    armDirectWorkConversationForChat();
                    return;
                }
    ...
    function createPanel() {
        ...
        const hint = createElement('div', { class: 'gpt-model-picker-hint' }, 'Оставаться в Chat явно передаёт conversation_origin=null и очищает Work execution target. Work → Chat восстанавливает штатный Chat → Work handoff, а для прямого Work готовит следующий ход с origin=null и после ответа проверяет server-side тип разговора.');
    ...
    window[GLOBAL_KEY] = {
        ...
        armDirectWorkConversationForChat,
        verifyDirectWorkChatAfterResponse,
