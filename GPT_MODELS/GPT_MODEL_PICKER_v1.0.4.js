...
(() => {
    ...
    const config = {
        ...
        /** Ключ штатного режима Chat / Work в localStorage ChatGPT. */
        homeComposerModeStorageKey: 'home-composer-mode-v1',
        ...
        /** Значение штатного composer mode для обычного Chat. */
        chatComposerMode: 'chat',

        /** Значение штатного composer mode для Work. */
        workComposerMode: 'work',
    ...
    /**
     * Записывает штатный home composer mode ChatGPT как Chat.
     *
     * Текущее значение home-composer-mode-v1 встречается в двух сериализациях:
     * raw-строка `work` и JSON-строка `"work"`. Метод сохраняет обнаруженный
     * формат. При отсутствующем ключе используется JSON-строка `"chat"`,
     * соответствующая JSON-персистентному значению composer mode.
     *
     * @returns {{ previousValue: string | null, nextValue: string, changed: boolean }}
     */
    function setNativeComposerModeToChat() {
        const previousValue = localStorage.getItem(config.homeComposerModeStorageKey);
        let nextValue;

        if (previousValue === config.chatComposerMode || previousValue === JSON.stringify(config.chatComposerMode)) {
            return {
                previousValue,
        ...
        if (previousValue === config.workComposerMode) {
            nextValue = config.chatComposerMode;
        } else if (previousValue === JSON.stringify(config.workComposerMode) || previousValue === null) {
            nextValue = JSON.stringify(config.chatComposerMode);
        } else {
            throw new Error(
                `${config.homeComposerModeStorageKey}: неожиданное значение ${JSON.stringify(previousValue)}`
            );
        }

        localStorage.setItem(config.homeComposerModeStorageKey, nextValue);
    ...
    /**
     * Переключает прямой Work-разговор на штатный composer mode Chat и готовит
     * следующий исходящий ход к отправке через Chat-параметры.
     *
     * Прямой Work не содержит continue_in_work handoff, поэтому restore-chat и
     * undo к нему неприменимы. Для такого разговора сохраняется штатный
     * home-composer-mode-v1=chat, создаётся вкладочный маркер Chat и выполняется
     * перезагрузка страницы, чтобы frontend перечитал composer mode и лимиты.
     *
     * @param {string} conversationId
     */
    function switchDirectWorkConversationToChat(conversationId) {
        const modeChange = setNativeComposerModeToChat();

        setDirectWorkChatConversation(conversationId);
        setStatus(
            state.restoreChatStatus,
            `Chat: прямой Work → Chat; composer ${modeChange.previousValue ?? 'не задан'} → ${modeChange.nextValue}; перезагрузка…`,
            'success'
        );
     ...
     * Прямой Work с conversation_origin=tpp не имеет handoff; для него метод
     * переводит home composer mode в Chat, сохраняет вкладочный Chat override и
     * перезагружает frontend. Следующий ход очищается от Work execution target.
     *
    ...
    async function restoreWorkConversationToChat() {
        ...
        try {
            ...
            if (!result) {
                if (handoff.conversation?.conversation_origin === 'tpp') {
                    setStatus(state.restoreChatStatus, 'Chat: прямой Work без handoff; переключение composer…', 'neutral');
                    switchDirectWorkConversationToChat(conversationId);
                    return;
                }

                setStatus(state.restoreChatStatus, 'Chat: разговор не является Work и handoff отсутствует', 'warning');
                return;
            }
