...
(() => {
    ...
    const config = {
        ...
        /** URL изменения свойств существующего разговора. */
        conversationUpdatePath: '/backend-api/conversation',
     ...
     * Это влияет на исходящий ход и не меняет server-side conversation_origin
     * уже существующего Work-разговора. Gizmo-режимы не преобразуются.
     *
     * @param {string} body
     ...
     * Server-side тип уже существующего Work-разговора этим переключателем не меняется.
     *
     * @param {boolean} enabled
     * @param {boolean} persist
     */
    ...
    /**
     * Запрашивает server-side перевод прямого Work-разговора в обычный Chat.
     *
     * Обычные Chat-разговоры в sidebar имеют conversation_origin=null, а прямой
     * Work имеет conversation_origin=tpp. Метод отправляет PATCH существующего
     * разговора с conversation_origin=null и после ответа повторно читает server-side
     * карточку. Успех признаётся только после подтверждения, что origin больше не tpp.
     *
     * @param {string} conversationId
     * @returns {Promise<void>}
     */
    async function convertDirectWorkConversationToChat(conversationId) {
        await requestAuthenticatedJson(
            `${config.conversationUpdatePath}/${encodeURIComponent(conversationId)}`,
            {
                method: 'PATCH',
                body: JSON.stringify({
        ...
        if (!conversationSummary) {
            throw new Error('После PATCH разговор не найден в server-side списке');
        }

        if (conversationSummary.conversation_origin === 'tpp') {
            throw new Error('Backend сохранил conversation_origin=tpp после PATCH');
        }

        setStatus(
            state.restoreChatStatus,
            `Chat: server origin ${conversationSummary.conversation_origin ?? 'null'}; перезагрузка…`,
            'success'
        );
     ...
     * Для прямого Work с conversation_origin=tpp выполняется server-side PATCH
     * conversation_origin=null. Перезагрузка выполняется только после повторной
     * проверки списка разговоров и подтверждения, что backend больше не возвращает tpp.
     *
     * @returns {Promise<void>}
     */
        ...
        try {
            ...
            if (!result) {
                ...
                if (conversationOrigin === 'tpp') {
                    setStatus(
                        state.restoreChatStatus,
                        `Chat: прямой Work; server origin tpp${conversationSummary ? ' подтверждён списком разговоров' : ''}; server-side PATCH → Chat…`,
                        'neutral'
                    );
                    await convertDirectWorkConversationToChat(conversationId);
                    return;
                }
    ...
    function createPanel() {
        ...
        const hint = createElement('div', { class: 'gpt-model-picker-hint' }, 'Оставаться в Chat очищает Work-параметры нового хода обычного Chat. Work → Chat восстанавливает штатный Chat → Work handoff, а для прямого Work с server origin tpp запрашивает server-side conversation_origin=null и проверяет результат перед перезагрузкой.');
    ...
    window[GLOBAL_KEY] = {
        ...
        convertDirectWorkConversationToChat,
